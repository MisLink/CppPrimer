...
public:
    QueryResult(std::string s, std::shared_ptr<StrVec> f, std::shared_ptr<std::set<TextQuery::line_no>> l) :sought(s), file(f), lines(l) { }
};
// class TextQuery

TextQuery::TextQuery(std::ifstream & is) :file(std::make_shared<StrVec>( )) {
    std::string text;
